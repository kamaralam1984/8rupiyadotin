...
export default function AgentDashboard() {
  ...
  const fetchUser = async () => {
    try {
      ...
      if (!response.ok || data.user.role !== "agent") {
        router.push("/login");
        return;
      }
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6 md:py-12">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-zinc-400">
              Welcome, {user?.name} ({user?.agentCode})
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-br from-zinc-800/40 via-zinc-800/30 to-zinc-900/40 p-6 backdrop-blur-xl">
            <h3 className="mb-4 text-xl font-semibold text-white">My Shops</h3>
            <p className="text-zinc-400">View and manage your shops</p>
          </div>

          <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-br from-zinc-800/40 via-zinc-800/30 to-zinc-900/40 p-6 backdrop-blur-xl">
            <h3 className="mb-4 text-xl font-semibold text-white">Register Shop</h3>
            <p className="text-zinc-400">Register a new shop</p>
          </div>

          <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-br from-zinc-800/40 via-zinc-800/30 to-zinc-900/40 p-6 backdrop-blur-xl">
            <h3 className="mb-4 text-xl font-semibold text-white">Earnings</h3>
            <p className="text-zinc-400">View your commission</p>
          </div>
        </div>
      </div>
    </div>
